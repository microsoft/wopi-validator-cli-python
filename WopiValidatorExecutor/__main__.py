...
def main():
    ...
    config_file = resource_filename('WopiValidatorExecutor.WopiValidatorExecutor', 'ExecutorConfig.ini')
    config.read(config_file)

    default_wopi_discovery_service_url = config['DEFAULT'][DEFAULT_WOPIDISCOVERYSERVICEURL]
    default_test_category = config['DEFAULT'][DEFAULT_TESTCATEGORY]
    ...
    parser.add_argument('-c', type=str, default=default_test_category,
                        help="Defines the target TestCategory, It could either be All, OfficeOnline or OfficeNativeClient and is case-insensitive.")
    parser.add_argument('-s', type=str, default=default_wopi_discovery_service_url,
